...
public class ForwardSliceHandlerTests
{
    [Fact]
    public async Task Handle_SimpleAssignment_ReturnsDependentRanges()
    {
...
class Sample
{
    int Compute(int input)
    {
        ...
        var yDeclarator = root.DescendantNodes().OfType<VariableDeclaratorSyntax>().First(d => d.Identifier.Text == "y");
        var zDeclarator = root.DescendantNodes().OfType<VariableDeclaratorSyntax>().First(d => d.Identifier.Text == "z");
        var text = await tree.GetTextAsync(CancellationToken.None);
        var focusPosition = text.Lines.GetLinePosition(yDeclarator.Identifier.SpanStart);
        var zAssignmentLine = text.Lines.GetLinePosition(zDeclarator.SpanStart).Line;
        ...
        var handler = new ForwardSliceHandler(orchestrator, NullLogger<ForwardSliceHandler>.Instance);

        var request = new ForwardSliceRequest
        {
            TextDocument = new TextDocumentIdentifier
            {
                Uri = DocumentUri.FromFileSystemPath(filePath)
            },
            Position = new Position(focusPosition.Line, focusPosition.Character)
        };

        var response = await handler.Handle(request, CancellationToken.None);

        response.Should().NotBeNull();
        response!.Direction.Should().Be(SliceDirection.Forward);
        response.SliceRanges.Should().NotBeEmpty();
        response.SliceRanges.Should().Contain(range => range.Start.Line == zAssignmentLine);
    }
}
